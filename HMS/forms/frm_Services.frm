...
Begin VB.Form frm_Services 
   ...
   End
End
Attribute VB_Name = "frm_Services"
...
On Error GoTo err:
    If MsgBox("Saving selected item cannot be undo." & vbCrLf & vbCrLf & "Are you sure?", vbQuestion + vbYesNo, "Confirm") = vbYes Then
        ...
        rs.Open "Select * from tblAddOthers", cn, adOpenKeyset, adLockPessimistic
            For x = 1 To Me.lvwListSel.ListItems.Count
                    rs.AddNew
                    rs.Fields("TransID").Value = transID
                    rs.Fields("Services").Value = Me.lvwListSel.ListItems(x).Text
                    rs.Fields("Price").Value = Me.lvwListSel.ListItems(x).SubItems(1)
...
On Error GoTo err:
    If rs.State = adStateOpen Then rs.Close
    rs.Open "Select * from tblServices", cn, adOpenKeyset, adLockPessimistic
    Do While rs.EOF = False
        Me.lvwList.ListItems.Add , , rs.Fields("sServices").Value
        Me.lvwList.ListItems(Me.lvwList.ListItems.Count).SubItems(1) = FormatNumber(rs.Fields("sPrice").Value, 2)
        rs.MoveNext
    Loop
    Exit Sub
err:
...
On Error GoTo err:
    ...
    If rs.State = adStateOpen Then rs.Close
    rs.Open "Select * from tblAddOthers  where TransID = " & transID, cn, adOpenKeyset, adLockPessimistic
    Me.lvwlistAdd.ListItems.Clear
    Do While rs.EOF = False
        Me.lvwlistAdd.ListItems.Add , , rs.Fields("Services").Value
        Me.lvwlistAdd.ListItems(Me.lvwlistAdd.ListItems.Count).SubItems(1) = FormatNumber(rs.Fields("Price").Value, 2)
